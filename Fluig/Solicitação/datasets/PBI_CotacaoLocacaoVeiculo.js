function createDataset(fields, constraints, sortFields) {
	...
	//constraintsFormulario.push(DatasetFactory.createConstraint("solicitacao", "7407", "7407", ConstraintType.MUST)); 
	var formularioSolicitacao = DatasetFactory.getDataset("VM_SolicitacoesLocacaoVeiculo", null, constraintsFormulario, null);
	...
	for (var a = 0; a < formularioSolicitacao.rowsCount; a++) {
		...
		if(formularioSolicitacao.getValue(a, "cotacaovalorTotal") != undefined && formularioSolicitacao.getValue(a, "cotacaovalorTotal") != null && formularioSolicitacao.getValue(a, "cotacaovalorTotal") != ""){
			valorCotacao = formularioSolicitacao.getValue(a, "cotacaovalorTotal");
			//valorCotacao = ;
		}
				...
				for (var b=0;  b < aprovacoes.rowsCount; b++){
					...
					if (ultimaAprovacao.length >0){
						...
						formularioAprovacao = DatasetFactory.getDataset("VM_SolicitacaoAprovacaoServico", null, constraintsAprovacao, null);
							...
							if (formularioAprovacao.getValue(0,"condicaoPgto") != "" && formularioAprovacao.getValue(0,"condicaoPgto") != null && formularioAprovacao.getValue(0,"condicaoPgto") != undefined){
								var condPagamento = formularioAprovacao.getValue(0,"condicaoPgto").split('-');  
							...
							if (retornaAprovadorNivel7.rowsCount > 0) {
								...
								if (dtAprovacaoNivel7 != null && dtAprovacaoNivel7 != "" && dtAprovacaoNivel7 != undefined){
									...
									if (dtAprovacaoNivel7 > ultimaDtAprovacao ){
					...
					if (numContrato == "" || numContrato == null || numContrato == undefined){
						...
						dataset.addRow(new Array(
								...
								acao_aprovador7,
								contratoPrevio,
								formularioSolicitacao.getValue(a, "codigoProduto"),
								formularioSolicitacao.getValue(a, "quantidade"),
								0,
								ultimaDtAprovacao
							));
					}
	...
	//Busca todos os formulários de transfer
	var constraintsFormulario = new Array();
	constraintsFormulario.push(DatasetFactory.createConstraint("metadata#active", true, true, ConstraintType.MUST));
	//constraintsFormulario.push(DatasetFactory.createConstraint("solicitacao", "7407", "7407", ConstraintType.MUST)); 
	formularioSolicitacao = DatasetFactory.getDataset("VM_SolicitacoesTransfer", null, constraintsFormulario, null);
