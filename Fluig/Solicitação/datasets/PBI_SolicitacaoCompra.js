function createDataset(fields, constraints, sortFields) {
	...
	//constraintsFormulario.push(DatasetFactory.createConstraint("solicitacao", "14150", "14150", ConstraintType.MUST)); 
	var formulario = DatasetFactory.getDataset("VM_SolicitacoesCompra", null, constraintsFormulario, null);
	...
	for (var a = 0; a < formulario.rowsCount; a++) {
		...
		if (processo.rowsCount > 0) {
			...
			//if (acao_aprovador1 != "" && acao_aprovador1 != undefined && acao_aprovador1 != null){
				...
				if (parseInt(versaoProcesso) >= 3) {
					//CONSULTA TAREFA APROVAÇÃO NIVEL 2
					retornaAprovadorNivel2 = consultaAprovacao(solicitacao, 50);
					if (retornaAprovadorNivel2.rowsCount > 0) {
						...
						for (var i=0; i < retornaAprovadorNivel2.rowsCount; i++){
							 ...
							 if (parseInt(ultima_acao_nivel2) > acao_atual_nivel2){
								 ...
								 usuarioAprovadorNivel2 = retornaAprovadorNivel2.getValue(i, "processTaskPK.colleagueId");
							 }
							 else {
					...
					retornaAprovadorNivel3 = consultaAprovacao(solicitacao, 51);
					if (retornaAprovadorNivel3.rowsCount > 0) {
						...
						for (var i=0; i < retornaAprovadorNivel3.rowsCount; i++){
							 ...
							 if (parseInt(ultima_acao_nivel3) > acao_atual_nivel3){
								 ...
								 usuarioAprovadorNivel3 = retornaAprovadorNivel3.getValue(i, "processTaskPK.colleagueId");
							 }
							 else {
				...
				constraintsItensFormulario.push(DatasetFactory.createConstraint("tablename", "tableCompras", "tableCompras", ConstraintType.MUST));
				...
				//Busca o dataset
				var itensFormulario = DatasetFactory.getDataset("VM_SolicitacoesCompra", null, constraintsItensFormulario, null);
				for (var j = 0; j < itensFormulario.rowsCount; j++) {
					var valorTotalProduto = itensFormulario.getValue(j, "vrTotUnit");
					valorTotalProduto = parseFloat(valorTotalProduto);
					
					//Adiciona os valores nas colunas respectivamente.
					dataset.addRow(new Array(
						itensFormulario.getValue(j, "txtproduto"),
						itensFormulario.getValue(j, "idum"),
						itensFormulario.getValue(j, "idquantidade"),
						itensFormulario.getValue(j, "vrUltima"),
						itensFormulario.getValue(j, "dtNecessidade"),
